...
public class ExpandSynapsedToTest extends OboFormatTestBasics {
	...
	public void testExpand() throws Exception {
		...
		OWLClass cls = df.getOWLClass(IRI.create("http://purl.obolibrary.org/obo/FBbt_00005123"));
		//Set<OWLClassAxiom> axioms = gciOntology.getAxioms(cls);
		...
		for (OWLEquivalentClassesAxiom eca : equivalentClassesAxioms) {
			...
			OWLObjectPropertyExpression p = df.getOWLObjectProperty(IRI.create("http://purl.obolibrary.org/obo/RO_0002104"));
			...
			if (ces.contains(cet4)) {
				...
				assertEquals("ObjectSomeValuesFrom(<http://purl.obolibrary.org/obo/BFO_0000051> ObjectIntersectionOf(<http://purl.obolibrary.org/obo/GO_0005886> ObjectSomeValuesFrom(<http://purl.obolibrary.org/obo/BFO_0000051> <http://purl.obolibrary.org/obo/TEST_4>)))", clst4ex.toString());
			}else if (ces.contains(cet5)) {
				ces.remove(cet5);
				OWLClassExpression clst5ex = ces.iterator().next();
				assertEquals("ObjectSomeValuesFrom(<http://purl.obolibrary.org/obo/BFO_0000051> ObjectIntersectionOf(<http://purl.obolibrary.org/obo/GO_0005886> ObjectSomeValuesFrom(<http://purl.obolibrary.org/obo/BFO_0000051> <http://purl.obolibrary.org/obo/TEST_5>)))", clst5ex.toString());
			}
			else {
