	...
	private void appendQualifiers(StringBuilder sb, Clause clause) {
		...
		if (qvs != null && qvs.size() > 0) {
			...
			if (qvs.size() > 1) {
				...
				List<QualifierValue> sortQualifiers = new ArrayList<QualifierValue>(qvs);
...
//			}
	...
	private static class HeaderTagsComparator implements Comparator<String>{
		...
		private static Hashtable<String, Integer> buildTagsPriorities(){
			...
			table.put(OboFormatTag.TAG_IMPORT.getTag(),80); // moved from pos 30 to emulate OBO-Edit behavior
	...
	private static class QualifierComparator implements Comparator<QualifierValue> {
