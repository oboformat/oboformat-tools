...
public class OBORunner {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // in the main method.
        // If the -b option is set, then build ALL ontologies in the specified
        // directory
        if (buildDir != null) {
    ...
    protected static void runConversion(OBORunnerConfiguration config,
            Logger logger, OWLOntologyManager manager,
            OWLOntologyManager outputManager) throws Exception {
        ...
        for (String iri : config.paths.getValue()) {
            ...
            if (config.isOboToOwl.getValue()) {
                ...
                if (outputFile == null) {
                    ...
                    outputFile = new File(config.outputdir.getValue(),
                            ontologyId + ".owl").getAbsolutePath();
                }
                ...
                if (config.writeLabelOWL.getValue()) {
                    ...
                    IRI labelFile = IRI.create(new File(config.outputdir
                            ...
                            .getCanonicalFile());
                    logger.info("saving " + ontologyId + " additional OWL to "
                            + outputStream);
                    manager.saveOntology(ontology, labelFormat, labelFile);
                }
            } else {
                OWLOntology ontology = manager
                        .loadOntologyFromOntologyDocument(IRI.create(iri));
                String version = config.version.getValue();
                if (version != null) {
                    addVersion(ontology, version, manager);
                }
                ...
                writer.close();
                final String untranslatableAxiomsFile = config.untranslatableAxiomsFile
                        .getValue();
                Collection<OWLAxiom> untranslatableAxioms = bridge
                        .getUntranslatableAxioms();
                if (untranslatableAxiomsFile != null
                        && untranslatableAxioms != null
                        && !untranslatableAxioms.isEmpty()) {
    ...
    private static OBODoc loadObo(OBORunnerConfiguration config, String iri,
            Logger logger) throws IOException,
            OBOFormatDanglingReferenceException, OBOFormatParserException {
        ...
        if (!config.allowDangling.getValue() && !errors.isEmpty()) {
            throw new OBOFormatDanglingReferenceException(
                    "Dangling references are found during conversion");
        }
    ...
    private static void usage() {
        System.out
                .println("obolib-obo2owl [--to SYNTAX, --allowdangling --followimports --strictconversion] -o FILEPATH-URI OBO-FILE");
        System.out.println("obolib-obo2owl -b BUILDPATH-URI");
        System.out.println("\n");
        System.out
                .println("Converts obo files to OWL. If -b option is used, entire\n");
    ...
    public static void showMemory() {
        ...
        long mem = tm - fm;
        System.out.println("Memory total:" + tm + " free:" + fm + " diff:"
                + mem + " (bytes) diff:" + mem / 1000000 + " (mb)");
    }

    /**
     * makes OWL from all selected ontologies. These are downloaded from the OBO
     * metadata file
     * 
     * @param dir
     * @param config
    ...
    protected static void buildAllOboOwlFiles(String dir,
        ...
        for (String ont : ontmap.keySet()) {
            ...
            if (omitOntsToDownload.size() > 0
                    && omitOntsToDownload.contains(ont)) {
                continue;
            }
            if (ontmap.containsKey(ont)) {
                ...
                try {
                    ...
                    logger.info("converting: " + ont + " from: " + url
                            + " using default ont:" + ontId);
                    if (url == null) {
    ...
    private static Map<String, String> getOntDownloadMap() throws IOException {
        return getOntDownloadMap(new URL(
    ...
    private static Map<String, String> getOntDownloadMap(BufferedReader in)
            throws IOException {
